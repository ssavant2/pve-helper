...
class StorageEnrollmentBoundaryTests(TestCase):
    ...
    def test_enrolling_a_node_writes_no_storage_consumer(self):
        """Consumer rows are operator policy about a host mount, not a side effect.

        Deriving one from enrollment would make the shared-mount gate agree with
        itself by construction: the evidence that a node was inventoried would be
        generated by the act of deciding to read it. Source-level, because the
        failure is a future call site that looks helpful.
        """
        root = Path(settings.BASE_DIR) / "core"
        modules = (
            root / "services" / "cluster_enrollment.py",
            root / "services" / "publication_scope.py",
            root / "views" / "clusters" / "enrollment.py",
        )

        offenders = [
            str(path.relative_to(settings.BASE_DIR))
            for path in modules
            if "ProxmoxStorageConsumer" in path.read_text(encoding="utf-8")
        ]
