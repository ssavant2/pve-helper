...
def rename_storage_file(
    *,
    ...
    root = _storage_root(storage)
    original_path = _storage_existing_file(entry.path, root=root)
    target_path = original_path.with_name(safe_name)
    if target_path.exists():
        raise StorageActionError("Target file already exists.")
    if not target_path.parent.is_relative_to(root):
    ...
    return {
        ...
        "new_path": _join_relative(parent_relative, safe_name),
