...
Membership, node runtime, node interfaces and their coverage are reconstructible
current state. Retirement removes them together so a retired connection cannot keep
looking partly live, while hard deletion accounts for the same relations through its
generic reconstructible-state teardown.
"""
...
def retire_cluster_projection(cluster) -> ClusterProjectionRetirementResult:
    ...
    interface_rows_deleted = ClusterNodeInterface._base_manager.filter(cluster_id=cluster.pk).delete()[0]
    ...
    return ClusterProjectionRetirementResult(
        ...
        interface_rows_deleted=interface_rows_deleted,
    )
