...
def guest_ref_from_audit_event(event: AuditEvent) -> GuestRef:
    ...
    try:
        ...
        raise DurableGuestOperationError("Audit event has no cluster-qualified guest target.") from exc
...
def cluster_for_guest_ref(ref: GuestRef) -> ProxmoxCluster:
    ...
    if cluster is None:
        raise DurableGuestOperationError(f"The Proxmox cluster '{ref.cluster_key}' no longer exists.")
    return cluster
...
def client_for_audit_event(event: AuditEvent, *, preferred_endpoint_url: str = ""):
    ...
    if not clients:
        raise DurableGuestOperationError(f"Cluster '{cluster.key}' has no enabled Proxmox endpoint.")
