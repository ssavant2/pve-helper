...
def parse_ovf_package(storage: StorageMount, source_path: str, *, validate_manifest: bool = False) -> OvfPackage:
    ...
    """
    relative_source = _normalized_relative_path(source_path)
    source = PurePosixPath(relative_source)
    ...
    if package_name == "imported-vm":
        package_name = _safe_name(source.stem)
    return OvfPackage(
...
def _normalized_relative_path(raw: str) -> str:
    try:
        return normalized_relative_path(raw, replace_backslashes=True)
    except ConfinedFilesystemError as exc:
        raise OvfImportError("Package path is invalid.") from exc
...
def _safe_member_name(raw: str) -> str:
    try:
        ...
        raise OvfImportError("Package contains an unsafe member path.") from exc
...
def _read_ova(root: str, relative_path: str, *, validate_manifest: bool) -> tuple[bytes, set[str], str | None]:
    try:
        with open_regular_file(root, relative_path) as package_handle:
            with tarfile.open(fileobj=package_handle, mode="r:*") as archive:
    ...
    except (ConfinedFilesystemError, tarfile.TarError, OSError) as exc:
        raise OvfImportError("Could not safely read the OVA archive.") from exc


def _read_ovf(root: str, relative_path: str) -> tuple[bytes, str | None]:
    try:
        with open_regular_file(root, relative_path) as handle:
            metadata = _read_limited(handle, os.fstat(handle.fileno()).st_size)
    except (ConfinedFilesystemError, OSError) as exc:
        raise OvfImportError("Could not safely read the OVF descriptor.") from exc
    manifest_path = PurePosixPath(relative_path).with_suffix(".mf").as_posix()
    manifest = _read_directory_manifest(root, manifest_path) if regular_file_exists(root, manifest_path) else None
    return metadata, manifest


def _read_directory_manifest(root: str, relative_path: str) -> str:
    try:
        with open_regular_file(root, relative_path) as handle:
            return _read_limited(handle, os.fstat(handle.fileno()).st_size).decode("utf-8", errors="strict")
    except (ConfinedFilesystemError, OSError, UnicodeDecodeError) as exc:
        raise OvfImportError("Could not safely read the OVF manifest.") from exc


def _directory_disk_members(root: str, source_path: str, disks: tuple[OvfDisk, ...]) -> set[str]:
    """Check only disks named by the descriptor; never walk the datastore."""
    members: set[str] = set()
    package_directory = PurePosixPath(source_path).parent
    for disk in disks:
        try:
            candidate = confined_relative_path(package_directory.as_posix(), disk.href)
        except ConfinedFilesystemError:
            continue
        if regular_file_exists(root, candidate):
...
def _validate_directory_manifest(root: str, source_path: str, manifest: str) -> None:
    package_directory = PurePosixPath(source_path).parent
    for algorithm, name, expected in _manifest_entries(manifest):
        try:
            candidate = confined_relative_path(package_directory.as_posix(), name)
