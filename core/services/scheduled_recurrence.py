...
def _daily_rule(recurrence: dict[str, Any], dtstart: datetime):
    ...
    return rrule(DAILY, dtstart=dtstart, byhour=hour, byminute=minute, bysecond=0)
...
def _weekly_rule(recurrence: dict[str, Any], dtstart: datetime):
    ...
    return rrule(WEEKLY, dtstart=dtstart, byweekday=byweekday, byhour=hour, byminute=minute, bysecond=0)


def _monthly_ordinal_rule(recurrence: dict[str, Any], dtstart: datetime):
    hour, minute = _time_parts(recurrence)
    ordinal = _ordinal_number(recurrence.get("ordinal", recurrence.get("week", "first")))
    weekday = WEEKDAYS[_weekday_number(recurrence.get("weekday", "monday"))](ordinal)
    return rrule(MONTHLY, dtstart=dtstart, byweekday=weekday, byhour=hour, byminute=minute, bysecond=0)
...
def _monthly_day_rule(recurrence: dict[str, Any], dtstart: datetime):
    ...
    return rrule(MONTHLY, dtstart=dtstart, bymonthday=day, byhour=hour, byminute=minute, bysecond=0)
